...
(function () {
  ...
  function renderDashboard() {
    ...
    if (vencRows.length) {
      ...
      var pySvg = "";
      var svgH = 320, svgW = 500, cx = svgW * 0.62;
      ...
      for (var pi = 0; pi < vencRows.length; pi++) {
        ...
        var wTop = pi === 0 ? 0 : Math.max(pctTop * maxW, 12);
        var wBot = Math.max(pctBot * maxW, 12);
        ...
        pySvg += '<text x="' + (labelX + 90) + '" y="' + (labelY + 9).toFixed(1) + '" text-anchor="end" fill="' + col + '" font-size="10" font-weight="600" font-family="system-ui,sans-serif">' + vencRows[pi].value + ' vencidos</text>';
      }
